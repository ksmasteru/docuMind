...
export default function AskChat({ compact = false }) {
  ...
  return (
    <div className="flex h-full flex-col bg-white dark:bg-slate-900">
      {isEmpty ? (
        <div className={`flex flex-1 flex-col items-center justify-center ${compact ? "px-3 text-center" : "px-4"}`}>
        ...
        <>
          <div className="flex-1 overflow-y-auto">
            <div className={compact ? "px-3 py-4" : "mx-auto max-w-3xl px-4 py-8"}>
                ...
                {messages.map((message, index) => {
                  ...
                  return (
                    <div key={index} className="flex items-start gap-3">
                      {!compact && (message.role === "user" ? <UserAvatar /> : <AssistantAvatar />)}

                      {message.role === "user" ? (
                        <div className={`ml-auto whitespace-pre-wrap rounded-2xl bg-slate-100 px-4 py-2.5 text-sm text-slate-900 dark:bg-slate-800 dark:text-slate-100 ${compact ? "max-w-full" : "max-w-[75%]"}`}>
                          {message.text}
                        </div>
                      ) : (
          ...
          <div className={`border-t border-slate-100 dark:border-slate-800 ${compact ? "px-3 py-3" : "px-4 py-4"}`}>
